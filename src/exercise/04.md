...
Adding .isRequired will throw an error if the prop is not passed
...
No propTypes are used in production, only development.

```jsx
Message.propTypes = {
  subject: PropTypes.string, // Value is validator function, returns error if problem with prop
  greeting: PropTypes.string,
}
```

If you use TypeScript, PropTypes are not really necessary.

React fragments allow us to return multiple components without a div container.
Can't have pass 2 separate things that are space-separated as 1 argument.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=kdaviduik%40gmail.com).
